...
test("Database helpers read and write rows", async () => {
    ...
    `)
		...
		expect(todos.map((todo) => todo.title)).toEqual(["Use Bun SQL", "Build database helpers"])
		...
		const deleted = await db.delete<TodoRow>("todos", { id: second!.id }, { returning: "*" })
...
test("Database validates identifiers", () => {
	...
	try {
		...
		expect(() => db.identifier("todos; drop table todos")).toThrow("Invalid SQL identifier")
	} finally {
