...
export type StoreUpdater<State> = Partial<State> | ((draft: State) => void | State | Partial<State>)
...
function pick<State extends object>(source: State, keys: readonly (keyof State)[] | undefined) {
	if (!keys) {
		return source
	}
...
export function createStore<State extends object>(schema: State, options?: StoreOptions<State>) {
